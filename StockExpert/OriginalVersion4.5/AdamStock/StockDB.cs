...
namespace StockExpert
{
	public class StockDB
	{
		...
		public static void CreateDBSchema()
		{
			...
			DataColumn[] columnArray1 = new DataColumn[1] { StockDB.dtSymbolList.Columns["Code"] } ;
			StockDB.dtSymbolList.PrimaryKey = columnArray1;
			...
			DataColumn[] columnArray2 = new DataColumn[1] { StockDB.dtFolderAll.Columns["FolderId"] } ;
			StockDB.dtFolderAll.PrimaryKey = columnArray2;
			...
			DataColumn[] columnArray3 = new DataColumn[2] { StockDB.dtFolderRel.Columns["Code"], StockDB.dtFolderRel.Columns["FolderId"] } ;
			StockDB.dtFolderRel.PrimaryKey = columnArray3;
			DataTable[] tableArray1 = new DataTable[3] { StockDB.dtSymbolList, StockDB.dtFolderAll, StockDB.dtFolderRel } ;
			StockDB.AllDataBase = tableArray1;
		}
		...
		public static DataPackage GetRealtimeData(DataRow dr)
		{
			...
			{
				return new DataPackage((DateTime) dr["LastTime"], (float) ((double) dr["OpenA"]), (float) ((double) dr["High"]), (float) ((double) dr["Low"]), (float) ((double) dr["CloseA"]), (double) dr["Volume"], (float) ((double) dr["CloseA"]));
			}
			return null;
		}
		...
		{
			DataRow row1 = StockDB.dtSymbolList.Rows.Find(Code);
			if (row1 != null)
			{
				StockDB.dtSymbolListChanged = true;
				if (dp.Last != 0f)
				...
				row1["High"] = dp.High;
				row1["Low"] = dp.Low;
				row1["CloseA"] = dp.Close;
				row1["Volume"] = dp.Volume;
				row1["LastTime"] = dp.Date;
				if (AlreadyMerged)
				{
					row1["MergeTime"] = dp.Date;
				}
				StockDB.UpdateRealtime(row1);
			}
		}
