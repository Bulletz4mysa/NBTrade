...
//		}
		...
		private void DrawStick(Graphics g,float x,float y,float h,Pen p,double ColumnWidth)
		{
			StickRenderType srt = Parent.StickRenderType;
			if (ColumnWidth>2 && (srt==StickRenderType.Column || srt==StickRenderType.ThickColumn))
			{
				int Space = 1;
				...
				g.DrawRectangle(p,(float)(x-ColumnWidth/2+Space),y,(float)(ColumnWidth-Space*2),h);
			} 
			else 
					...
					switch (fd.RenderType)
					{
						case FormulaRenderType.VOLSTICK:
							...
							for(int j=0; j<pfs.Length; j++) 
							{
								Pen P = CurrentPen;
								if (pfs2!=null)
									P = SetAlpha(i,Pens[pfs2[j].Y==Z?2:0]);
								DrawStick(g,pfs[j].X,pfs[j].Y,R.Bottom-pfs[j].Y,P,ColumnWidth);
							}
							break;
						case FormulaRenderType.COLORSTICK:
							...
							for(int j=0; j<pfs.Length; j++)
							{
								try
								{
									Pen P = Pens[pfs[j].Y<Zero?0:2];
									//PointF pfb = new PointF(pfs[j].X,Zero);
									DrawStick(g,pfs[j].X,pfs[j].Y,Zero-pfs[j].Y,P,ColumnWidth);
...
//							break;
		...
		{
			for(int i =0; i<FormulaDataArray.Count; i++)
				if ((this[i].RenderType == FormulaRenderType.STOCK || this[i].RenderType==FormulaRenderType.MONOSTOCK) && 
