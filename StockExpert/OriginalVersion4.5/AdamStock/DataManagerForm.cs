...
{
    public delegate void DoChangeUIText(Control control, string text);
    public delegate void InitializeProgressBar(ProgressBar control, Int32 value);
    public delegate void UpdateProgressBar(ProgressBar control, Int32 value);
	public class DataManagerForm : Form
	{
        const String YahooDownLoadLink ="http://table.finance.yahoo.com/table.csv?s={0}&d={4}&e={5}&f={6}&g=d&a={1}&b={2}&c={3}&ignore=.csv";
		// Methods
        ...
        private void CrossThreadChangeUIText(Control control, String text)
        {
            if (control.InvokeRequired)
            {
                DoChangeUIText theDelegate = CrossThreadChangeUIText;
                control.Invoke(theDelegate, new object[] {control, text });
            }
            else
            {
                control.Text = text;
        ...
        private void InitializeProgressBarValue(ProgressBar control, Int32 value)
        {
            if (control.InvokeRequired)
            {
                InitializeProgressBar theDelegate = InitializeProgressBarValue;
                control.Invoke(theDelegate, new object[] { control, value });
            }
            else
            {
        ...
        private void UpdateProgressBarValue(ProgressBar control, Int32 value)
        {
            if (control.InvokeRequired)
            {
                UpdateProgressBar theDelegate = UpdateProgressBarValue;
                control.Invoke(theDelegate, new object[] { control, value });
            }
            else
            {
		...
		{
		    int num1 = -1;
            if (cbMode.InvokeRequired)
            {
                cbMode.Invoke(new MethodInvoker(delegate { num1 = this.cbMode.SelectedIndex; }));
            }
            else
            {
                num1 = cbMode.SelectedIndex;
            }
		    string text1 = YahooDownLoadLink;
			WebClient client1 = new WebClient();
			string[] textArray1 = ListForm.Current.GetSymbolList();
			int num2 = 0;
			InitializeProgressBarValue(pbDownload, textArray1.Length);
            UpdateProgressBarValue(pbDownload, 0);
			Application.DoEvents();
			try
			{
				...
				for (int num3 = 0; num3 < textArray2.Length; num3++)
				{
					string text2 = textArray2[num3];
					try
					{
						DateTime time1 = this.dpStart.Value;
						DateTime time2 = this.dpStop.Value;
						CommonDataProvider provider1 = (CommonDataProvider) manager1[text2];
						if ((num1 == 0) && (provider1.Count > 0))
						{
							double[] numArray1 = provider1["DATE"];
							DateTime time3 = DateTime.FromOADate(numArray1[numArray1.Length - 1]);
							time1 = time3.AddDays(1);
						}
						if (time1 <= DateTime.Now.Date)
						{
							object[] objArray1 = new object[7] { text2, time1.Month - 1, time1.Day, time1.Year, time2.Month - 1, time2.Day, time2.Year } ;
							string text3 = string.Format(text1, objArray1);
							byte[] buffer1 = client1.DownloadData(text3);
							CommonDataProvider provider2 = YahooDataManager.LoadYahooCSV((IDataManager) null, buffer1);
							if (provider1 != null)
							{
								provider2.Merge(provider1);
							}
							provider2.SaveBinary(FileDataManager.GetFileName(text2));
							this.UpdateRealtimeFromHistorical(text2, provider2);
						}
					}
					catch (Exception ex)
					{
                    ...
                    UpdateProgressBarValue(pbDownload, num2);
				    CrossThreadChangeUIText(lDownload, num2 + "/" + textArray1.Length);
					Application.DoEvents();
				}
			}
			finally
			{
                CrossThreadChangeUIText(btnBeginDownload, "&Start Download");
				this.DownloadHistoryThread = null;
				this.EnableDisableActions(true);
			}
		}
		...
		public void DownloadRealtime()
		{
			string[] textArray1 = ListForm.Current.GetSymbolList();
			int num1 = 0;
			this.pbRealtime.Maximum = textArray1.Length;
			this.pbRealtime.Value = 0;
			this.lProgress.Text = "0/" + textArray1.Length;
			Thread.Sleep(1);
			try
			{
				string[] textArray2 = textArray1;
				for (int num2 = 0; num2 < textArray2.Length; num2++)
				{
					...
					try
					{
						DataPackage package1 = DataPackage.DownloadFromYahoo(text1);
						if (!package1.IsZeroValue)
						{
							StockDB.LoadRealtimeRow(text1, package1, false);
						}
					}
					catch
					{
					...
					this.pbRealtime.Value = num1;
						
					this.lProgress.Text = num1 + "/" + textArray1.Length;
					Application.DoEvents();
				}
			}
			finally
		...
		private void InitializeComponent()
		{
			...
			this.tsDataService.Text = "Easychart Data Service";
			// 
			// lData
			// 
			...
			this.lData.Size = new System.Drawing.Size(24, 17);
			...
			this.lDesc.Size = new System.Drawing.Size(546, 20);
			...
			this.lExchange.Size = new System.Drawing.Size(64, 17);
			...
			this.lProgress.Size = new System.Drawing.Size(24, 17);
			...
			this.lMode.Size = new System.Drawing.Size(40, 17);
			...
			this.lStopDate.Size = new System.Drawing.Size(66, 17);
			...
			this.lStartDate.Size = new System.Drawing.Size(68, 17);
			...
			this.lDownload.Size = new System.Drawing.Size(24, 17);
		...
		{
			this.DownloadHistoryThread = new Thread(new ThreadStart(this.DownloadHistory));
			this.DownloadHistoryThread.Start();
