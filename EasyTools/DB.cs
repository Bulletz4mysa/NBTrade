...
namespace EasyTools
{
	...
	public abstract class BaseDb 
	{
		...
		protected string ReplaceParam(string sql,DbParam[] dp) 
		{
		...
		public static BaseDb GetBaseDb(string ConnStr) 
		{
			...
			{
				//#EasyMySql.dll | server={0};user id={1}; password={2}; database=mysql; pooling=false
				string[] ss = ConnStr.Split('|');
				if (ss.Length==2)
				{
					foreach(Assembly a in AppDomain.CurrentDomain.GetAssemblies())
					{
						string[] sss = a.FullName.Split(',');
						if (string.Compare(sss[0].Trim(),ss[0].Substring(1).Trim(),true)==0)
						{
							foreach(Type t in a.GetTypes()) 
							{
								if (t.BaseType==typeof(BaseDb))
								{
									object o = Activator.CreateInstance(t,new object []{ss[1].Trim()});
									if (o!=null)
										return (BaseDb)o;
								}
							}
		...
		public static BaseDb Current 
		{
			get 
			{
				...
				if (bd==null) 
				{
					bd= FromConfig(DB.DefaultConnStr);// GetBaseDb(ConfigurationSettings.AppSettings["ConnStr"]);
					if (HttpContext.Current!=null)
	...
	public class DB
	{
		static public string DefaultConnStr = "ConnStr";
