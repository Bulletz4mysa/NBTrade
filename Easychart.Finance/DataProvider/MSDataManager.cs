...
namespace Easychart.Finance.DataProvider
{
			...
			using (FileStream fs =  ReadData(FilePath + "MASTER"))
			{
				...
				using (BinaryReader br = new BinaryReader(fs))
				{
					...
					do 
					{
						...
						Masters[i].time_frame = br.ReadByte();
	...
	public class Master
	{
		...
		public byte time_frame;									/* data format: 'I'(IDA)/'W'/'Q'/'D'/'M'/'Y' */
...
//	@Rowa A {120}	B {149}	C {30} 	D {Unknown}
	public class XMaster
	{
		...
		public byte[] Unknown2=new byte[3];				// 3 bytes
