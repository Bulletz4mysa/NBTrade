namespace Easychart.Finance
{
    ...
    public class FormulaArea
    {
        ...
        public FormulaArea(FormulaChart fc)
        {
            ...
            this.StockRenderType = Easychart.Finance.StockRenderType.Candle;
        ...
        private Pen[] AdjustPenWidth(int w)
        {
            if (this.BarPens != null)
            {
                ...
                {
                    this.StockRenderType = Easychart.Finance.StockRenderType.Bar;
                }
            ...
            else
            {
                ...
                try
                {
                    for (int i = 0; i < this.FormulaDataArray.Count; i++)
                    {
                        ...
                        FormulaAlign right;
                        PointF tf5;
                        FormulaData f = this.FormulaDataArray[i];
                        if (f.IsNaN())
                        {
                            continue;
                        }
                        double[] filter = null;
                        if ((((f.RenderType == FormulaRenderType.POLY) || (f.RenderType == FormulaRenderType.PARTLINE)) || ((f.RenderType == FormulaRenderType.FILLRGN) || (f.RenderType == FormulaRenderType.LINE))) || ((f.RenderType == FormulaRenderType.TEXT) || (f.RenderType == FormulaRenderType.STICKLINE)))
                        {
                            filter = f["COND"];
                        }
                        PointF[] points = f.GetPoints((f.RenderType == FormulaRenderType.PARTLINE) || (f.RenderType == FormulaRenderType.FILLRGN), filter);
                        ...
                        switch (f.RenderType)
                        {
                            ...
                            if (points.Length > 0)
                            {
                                double num20 = (double) f.OwnerData["START"];
                                double num21 = (double) f.OwnerData["END"];
                                float y = points[0].Y;
                                R = f.AxisY.FrameRect;
                                float num23 = (float) (R.X + num20);
                                float num24 = (float) (R.Right - num21);
                                if (f.AxisY.AxisPos == AxisPos.Left)
                                {
                            ...
                            case FormulaRenderType.VOLSTICK:
                                ...
                                data2 = this.DataProvider["CLOSE"];
                                if (this.StockRenderType != Easychart.Finance.StockRenderType.Bar)
                                {
                                    break;
                                }
                            ...
                            {
                                double[] dd = f["NUMBER"];
                                string text = (string) f.OwnerData["TEXT"];
                                object obj2 = f.OwnerData["FORMAT"];
                                string str2 = "";
                                if (obj2 != null)
                                {
                                    str2 = obj2.ToString();
                                }
                                PointF[] tfArray3 = f.GetPoints(dd, false, f["COND"], true);
                                for (int k = 0; k < points.Length; k++)
                                {
                                    ...
                                    FormulaLabel label = this.Labels[f.LabelIndex % this.Labels.Length];
                                    Brush brush = label.TextBrush;
                                    if (brush == null)
                                    {
                                        brush = new SolidBrush(currentPen.Color);
                                    }
                                    label.DrawString(g, text, this.TextFont, brush, f.VAlign, f.Align, pos, f.VAlign != VerticalAlign.VCenter);
                                }
                                continue;
                            }
                            case FormulaRenderType.AXISYTEXT:
                                ...
                                str3 = (string) f.OwnerData["TEXT"];
                                num25 = (float) ((double) f.OwnerData["START"]);
                                num26 = points[0].Y;
                                num27 = 0f;
                                R = f.AxisY.FrameRect;
                                right = f.Align;
                                if (f.AxisY.AxisPos != AxisPos.Right)
                                {
                                    goto Label_0F37;
                                }
                            ...
                            case FormulaRenderType.FILLRGN:
                                tfArray = f.GetPoints("PRICE2", true, "COND");
                                tfArray7 = f.GetPoints(true);
                                tfArray8 = f.GetPoints("PRICE2", true);
                                list = new ArrayList();
                                num18 = 0;
                                goto Label_0C93;
                            ...
                            case FormulaRenderType.LINE:
                                tfArray = f.GetPoints("PRICE2", false, "COND2");
                                num12 = 0;
                                goto Label_0885;
                            ...
                            {
                                if (this.StockRenderType != Easychart.Finance.StockRenderType.Line)
                                {
                                    goto Label_08B2;
                                }
                                this.DrawPoints(g, f, currentPen, points, ColumnWidth);
                                continue;
                            }
                            default:
                                goto Label_0FB2;
                    ...
                    Label_02F9:
                        data3 *= f.MaxValue(this.Canvas.Start, this.Canvas.Count);
                    ...
                    Label_055F:
                        numArray2 = f["PRICE2"];
                        PointF tf2 = new PointF(points[num8].X, f.AxisY.CalcY(numArray2[num8]));
                    ...
                    Label_06F0:
                        if (f["COND"][num10] == 1.0)
                        {
                            g.DrawString(f.OwnerData["ICON"].ToString(), new Font("Verdana", 9f), Brushes.Black, points[num10]);
                        }
                        num10++;
                    Label_074E:
                    ...
                    Label_0885:
                        ...
                        continue;
                    Label_08B2:
                        tfArray4 = f.GetPoints("H");
                        PointF[] tfArray5 = f.GetPoints("L");
                        PointF[] tfArray6 = f.GetPoints("O");
                        penArray = this.AdjustPenWidth(w);
                        for (int j = 0; j < points.Length; j++)
                        {
                            ...
                            if ((w <= 1) || (this.StockRenderType == Easychart.Finance.StockRenderType.Bar))
                            {
                    ...
                    Label_0D77:
                        if ((num19 == (points.Length - 1)) || points[num19].IsEmpty)
                        {
                            if (list3.Count > 0)
                            {
                                ...
                                this.DrawPoints(g, f, currentPen, tfArray, ColumnWidth);
                            }
                            list3.Clear();
                        }
                        else
                    ...
                    Label_0F47:
                        ...
                        FormulaLabel label2 = this.Labels[f.LabelIndex % this.Labels.Length];
                        Brush textBrush = label2.TextBrush;
                        if (textBrush == null)
                        {
                            textBrush = new SolidBrush(currentPen.Color);
                        }
                        label2.DrawString(g, str3, this.TextFont, textBrush, f.VAlign, right, tf5, f.VAlign != VerticalAlign.VCenter);
                        continue;
                    Label_0FB2:
                        this.DrawPoints(g, f, currentPen, points, ColumnWidth);
                    }
